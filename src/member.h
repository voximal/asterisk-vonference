...
// $Id: member.h,v 1.1 2016/05/26 08:45:43 borja.sixto Exp $
...
#ifndef _VONFERENCE_MEMBER_H
#define _VONFERENCE_MEMBER_H
...
#ifdef ulex
#ifdef	VIDEO
	struct ast_filestream *vstream; // the video stream
#endif
...
struct ast_conf_member
{
  ...
  int dsp_threshold;
	...
	// exit by dtmf
	short dtmf_exit;
	// allow to change mute/freeze by dtmf
	short dtmf_change;
	// unmute/show after a dtmf
	short dtmf_unmute;

	// Listen to speakers only (DigitalSamba)
	int listener_only;
	// Text commandes/events (DigitalSamba)
	int digitalsamba;
	
	// Prompts mute/unmute show/hide (DigitalSamba)
	int promptchange;
  ...
  struct ast_filestream *recordfilestream_in;
  struct ast_filestream *recordfilestream_out;
#endif
} ;
