...
static void conference_exec( struct ast_conference *conf )
{
    ...
    ast_mutex_lock(&conflist_lock);
      ...
      // get list of conference members
			...
			while ( member != NULL )
			{
	      struct ast_conf_member *spyee;

        if ( member->spyee_channel_name != NULL && member->spy_partner == NULL)
        {
		      spyee = member->spy_partner = find_member(member->spyee_channel_name);
		      if (spyee != NULL)
		      {
			     --spyee->use_count;
		        ast_log( LOG_NOTICE, "Found spyee member conference %s !\n", member->spyee_channel_name ) ;
          }
				    ...
				    )
				{
					...
					while ( cfr )
					{
                        ...
                        member->req_id == -1) ||
							    ...
							    if ( member->req_id == video_source_member->id )
								  {
									 queue_outgoing_video_frame(member, cfr->fr, conf->delivery_time);
								  }
#endif										
			...
			for (dtmf_source_member = conf->memberlist; dtmf_source_member != NULL; dtmf_source_member = dtmf_source_member->next)
			{
				while ((cfr = get_incoming_dtmf_frame( dtmf_source_member )))
				{
					for (member = conf->memberlist; member != NULL; member = member->next)
					{
						...
						{
#ifdef ulex
              if (cfr->fr->frametype == AST_FRAME_TEXT)
              {
              // Send the latest frame
							queue_outgoing_text_frame(member, cfr->fr);
...
struct ast_conference* find_member_conf( struct ast_conf_member *member )
{
	...
	while ( conf != NULL )
	{
			for (othermember = conf->memberlist; othermember != NULL; othermember = othermember->next) {
...
static struct ast_conference* create_conf( char* name, struct ast_conf_member* member )
{
	...
	ast_format_set(&dst, AST_FORMAT_ULAW, 0);
	conf->from_slinear_paths[ AC_ULAW_INDEX ] = ast_translator_build_path(&dst, &src);
	ast_format_set(&dst, AST_FORMAT_ALAW, 0);
	conf->from_slinear_paths[ AC_ALAW_INDEX ] = ast_translator_build_path(&dst, &src);
	ast_format_set(&dst, AST_FORMAT_GSM, 0);
	conf->from_slinear_paths[ AC_GSM_INDEX ] = ast_translator_build_path(&dst, &src);
#ifdef	AC_USE_SPEEX
	ast_format_set(&dst, AST_FORMAT_SPEEX, 0);
	conf->from_slinear_paths[ AC_SPEEX_INDEX ] = ast_translator_build_path(&dst, &src);
#endif
#ifdef AC_USE_G729A
	ast_format_set(&dst, AST_FORMAT_G729A, 0);
	conf->from_slinear_paths[ AC_G729A_INDEX ] = ast_translator_build_path(&dst, &src);
#endif
#else
...
#endif
#ifdef AC_USE_G722
	ast_format_set(&dst, AST_FORMAT_G722, 0);
  conf->from_slinear_paths[ AC_G722_INDEX ] = ast_translator_build_path(&dst, &src);
#endif
#endif
...
struct ast_conference *remove_conf( struct ast_conference *conf )
{
	...
	DEBUG("removed %s from the conference table, bucket => %d\n", conf->name, conf->bucket - conference_table) ;
	...
	while ( member_list != NULL )
	{
		...
		if ( member_list == member )
		{
			...
			if (conf->debug_flag)
			{
				...
				DEBUG("member accounting, channel => %s, te => %ld, fi => %ld, fid => %ld, fo => %ld, fod => %ld, tt => %ld\n", GET_CHAN_NAME(member->chan), member->time_entered.tv_sec, member->frames_in, member->frames_in_dropped, member->frames_out, member->frames_out_dropped, tt) ;
			}
#endif
			//
...
int show_conference_stats ( int fd )
{
	...
	ast_cli( fd, "%-40.40s %-20.20s %-20.20s %-20.20s\n", "Name", "Members", "Volume", "Duration" ) ;
				...
				else
        {
          if (member->spy_partner != NULL)
				...
				if ( member->driven_member == NULL )				
				{
					...
					member->id, member->flags, (member->mute_audio == 0 ? "Unmuted" : "Muted"), volume_str, "*", duration_str, spy_str, GET_CHAN_NAME(member->chan));
				} else {
					ast_cli( fd, "%-20d %-20.20s %-20.20s %-20.20s %-20d  %-20.20s %-20.20s %-80s\n", member->id, member->flags,
					(member->mute_audio == 0 ? "Unmuted" : "Muted"), volume_str, member->driven_member->id, duration_str, spy_str, GET_CHAN_NAME(member->chan));
				}
#else
				ast_cli( fd, "User %c%d %c:\n Flags: %s\n Audio/video: %s/%s\n Volume: %s\n Driver : #%d (%d:%d)\n Duration: %s\n Spy: %s\n Channel: %s\n Audio : in %ld/%ld, out %ld/%ld\n Video : in %ld/%ld, out %ld/%ld\n", ((conf->default_video_source_id == member->id) ? '*' : '#'), member->id, ((conf->current_video_source_id == member->id) ? '*' : ' '), member->flags,
				(member->mute_audio == 0 ? "Unmuted" : "Muted"), (member->mute_video == 0 ? "Unmuted" : "Muted"), 
        volume_str, (member->driven_member != NULL ? member->driven_member->id : -1) , member->initial_id, member->req_id, duration_str, spy_str, GET_CHAN_NAME(member->chan),
				member->frames_in_dropped, member->frames_in, member->frames_out_dropped, member->frames_out,
				member->video_frames_in_dropped, member->video_frames_in, member->video_frames_out_dropped, member->video_frames_out
				);
        
        if (member->vad_switch)
        ast_cli( fd, " VAD : %d/%d (%ld)\n Start timeout :%d\n Stop timeout :%d\n", member->local_speaking_state, member->speaking_state, (long)ast_tvdiff_ms(current_time, member->last_state_change), member->video_start_timeout, member->video_stop_timeout );
#endif				
#else
	...
	while ( conf != NULL )
	{
		if ( strcasecmp( (const char*)&(conf->name), confname ) == 0 )
		{
			...
			while (member != NULL)
              ...
              if ((member->promptchange) && (!member->mute_audio))
              {
  ...
  int stream_id = -1;
	...
	while ( conf != NULL )
	{
		if ( strcasecmp( (const char*)&(conf->name), confname ) == 0 )
		{
			...
			while (member != NULL)
			{
				if (strcasecmp( GET_CHAN_NAME(member->chan), streamchan ) == 0)
				{
					stream_id = member->id;
				}
				member = member->next;
			...
			if (stream_id >= 0)
			{
				...
				while (member != NULL)
				{
					if (strcasecmp( GET_CHAN_NAME(member->chan), userchan ) == 0)
					{
						// switch the video
						ast_mutex_lock( &member->lock ) ;
...
struct ast_conf_member *find_member( const char *chan )
{
  ...
  //ast_log(LOG_WARNING, "Find member (%s %d)\n", chan, chanid);

	AST_LIST_LOCK ( bucket ) ;

	AST_LIST_TRAVERSE ( bucket, member, hash_entry )
	{
		if (!strcmp (GET_CHAN_NAME(member->chan), chan) ) {
			ast_mutex_lock (&member->lock) ;
			member->use_count++ ;
			break ;
		}
    ...
    // acquire mutex
	  ...
	  {
			for (member = conf->memberlist; member != NULL; member = member->next) {
        //ast_log(LOG_WARNING, "Conference memberID %d (%s %d)\n", member->id, chan, chanid);

        char *number = GET_CHAN_CALLERID_NUMBER(member->chan);
		    if ((chan[0]=='@') && (!strcmp(number, chan+1) )) {
			    ast_mutex_lock (&member->lock) ;
			    member->use_count++ ;
			    conf = NULL;
		  	  break ;
		    ...
		    if ((member->id!=-1) && (member->id==chanid)) {
			    ast_mutex_lock (&member->lock) ;
			    member->use_count++ ;
			    conf = NULL;
			    break ;
...
int lock_conference_channel(const char *conference, const char *channel)
{
	...
	for ( conf = conflist ; conf != NULL ; conf = conf->next )
	{
		if ( strcmp(conference, conf->name) == 0 )
		{
			...
			for ( member = conf->memberlist ; member != NULL ; member = member->next )
			{
				if ( strcmp(channel, GET_CHAN_NAME(member->chan)) == 0 && !member->mute_video )
				{
	...
	for ( conf = conflist ; conf != NULL ; conf = conf->next )
	{
		if ( strcmp(conference, conf->name) == 0 )
		{
			...
			for ( member = conf->memberlist ; member != NULL ; member = member->next )
			{
				// We do not allow video muted members or members that do not support
				// VAD switching to become defaults
				if ( strcmp(channel, GET_CHAN_NAME(member->chan)) == 0 &&
				     !member->mute_video &&
				     member->vad_switch
				   )
				{
...
int video_mute_member(const char *conference, int member_id)
{
	...
	for ( conf = conflist ; conf != NULL ; conf = conf->next )
	{
        ...
        if (!member->no_camera)
          ...
          if ((member->promptchange) && (!member->mute_video))
          {
	          struct ast_conf_soundq *newsound;
	          struct ast_conf_soundq **q;
		...
		if ( strcmp(conference, conf->name) == 0 )
		{
			...
			for ( member = conf->memberlist ; member != NULL ; member = member->next )
			{
				if ( member->id == member_id )
				{
					...
					manager_event(EVENT_FLAG_CALL, "ConferenceChange", "ConferenceName: %s\r\nChannel: %s\r\n", conf->name, GET_CHAN_NAME(member->chan));
...
int send_text_channel(const char *conference, const char *channel, const char *text)
{
	...
	for ( conf = conflist ; conf != NULL ; conf = conf->next )
	{
		if ( strcmp(conference, conf->name) == 0 )
		{
			...
			for ( member = conf->memberlist ; member != NULL ; member = member->next )
			{
				if ( strcmp(GET_CHAN_NAME(member->chan), channel) == 0 )
				{
					res = send_text_message_to_member(member, text) == 0;
					break;
				}
...
int drive_channel(const char *conference, const char *src_channel, const char *dst_channel)
{
	...
	for ( conf = conflist ; conf != NULL ; conf = conf->next )
	{
		if ( strcmp(conference, conf->name) == 0 )
		{
			...
			for ( member = conf->memberlist ; member != NULL ; member = member->next )
			{
				if ( strcmp(src_channel, GET_CHAN_NAME(member->chan)) == 0 )
					src = member;
				if ( dst_channel != NULL && strcmp(dst_channel, GET_CHAN_NAME(member->chan)) == 0 )
					dst = member;
			}
			if ( src != NULL )
			{
	...
	if ( new_id != conf->current_video_source_id )
	{
      ...
      if ( member->id == conf->current_video_source_id )
			...
			if ( member->id == new_id )
			{
				...
				if (member->keep_focus)
				{
          conf->default_video_source_id = new_id;
        }
			}
			}
		}
		...
		manager_event(EVENT_FLAG_CALL,
			...
			"ConferenceName: %s\r\nChannel: %s\r\n",
			conf->name,
			new_member == NULL ? "empty" : GET_CHAN_NAME(new_member->chan)
			);

    if (conf->current_video_source_id != new_id)
		ast_log( LOG_NOTICE, "Video Switching  from %d to %d !\n", conf->current_video_source_id, new_id);
...
int play_sound_conference(int fd, const char *name, const char *file, int mute)
{
	...
	ast_cli(fd, "Playing sound %s to conference %s %s\n",
		      file, name, mute ? "with mute" : "");
...
int stop_sound_conference(int fd, const char *name)
{
	...
	ast_cli(fd, "Stoping sounds into the conference %s\n",
		      name);
...
int count_exec( struct ast_channel* chan, const char* data )
{
	...
	if (!ast_strlen_zero(args.varname)){
		...
		if (GET_CHAN_STATE(chan) != AST_STATE_UP)
			ast_answer(chan);
		res = ast_say_number(chan, count, "", GET_CHAN_LANGUAGE(chan), (char *) NULL);
	}
	return res;
}
