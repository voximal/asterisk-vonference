...
static int process_incoming(struct ast_conf_member *member, struct ast_conference *conf, struct ast_frame *f)
{
 ...
 int dspsilence = 0;
   ...
   //DEBUG("Process Icomming  frame_subclass(format) = %x\n", GET_FRAME_SUBCLASS_FORMAT(f));
	...
	{
#ifdef ulex
	  if (member->digitalsamba)
	  {
	    char dtmf[2];
	    char message[100];

      if ((!member->dtmf_switch) && (member->dtmf_change) && (GET_FRAME_CONTROL_TYPE(f)=='#'))
      {
	      dtmf[0]=GET_FRAME_CONTROL_TYPE(f);
	      dtmf[1]=0;
        ...
        if (member->promptchange)
        {
	        ...
	        struct ast_conf_soundq **q;
			    ...
			    if ((newsound = calloc(1, sizeof(struct ast_conf_soundq))))
			    {
			      ...
			      ast_copy_string(newsound->name, "mute", sizeof(newsound->name));
			      else
			      ast_copy_string(newsound->name, "unmute", sizeof(newsound->name));
				...
				sprintf(message, "onBroadcastChange,%d,%d,%d", member->id, !member->mute_audio, !member->mute_video);
				send_text_broadcast(conf->name, message);
	    }

      if ((!member->dtmf_switch) && (member->dtmf_change) && (!member->no_camera) && (GET_FRAME_CONTROL_TYPE(f)=='*'))
      {
	      char message[100];
	      dtmf[0]=GET_FRAME_CONTROL_TYPE(f);
	      dtmf[1]=0;
        ...
        if (member->promptchange)
        {
	        ...
	        struct ast_conf_soundq **q;
			    ...
			    if ((newsound = calloc(1, sizeof(struct ast_conf_soundq))))
			    {
			      ...
			      ast_copy_string(newsound->name, "hide", sizeof(newsound->name));
			      else
			      ast_copy_string(newsound->name, "show", sizeof(newsound->name));
      ...
      if (!member->dtmf_switch)
      {
	      dtmf[0]=GET_FRAME_CONTROL_TYPE(f);
	      dtmf[1]=0;
        pbx_builtin_setvar_helper(member->chan, "CONFERENCE_DTMF", dtmf);    
	      member->kick_flag=1;
	    }
    }
	  if (member->dtmf_unmute)
	  {
			member->mute_audio = 0;
			member->mute_video = 0;
			unmute_member(conf->name, member->req_id);
			video_unmute_member(conf->name, member->req_id);
    }
#endif
#ifdef	VIDEO
		if (member->dtmf_switch)
		{
			...
			switch (GET_FRAME_CONTROL_TYPE(f)) {
		...
		if (member->dtmf_relay)
		{
			...
			manager_event(
				...
				conf->name,
				member->type,
				GET_CHAN_UNIQUEID(member->chan),
		 		GET_CHAN_NAME(member->chan),
	    	GET_CHAN_CALLERID_NUMBER(member->chan) ? GET_CHAN_CALLERID_NUMBER(member->chan) : "unknown",
   		  GET_CHAN_CALLERID_NAME(member->chan) ? GET_CHAN_CALLERID_NAME(member->chan): "unknown",
				GET_FRAME_CONTROL_TYPE(f),
				conf->membercount,
				member->flags,
				member->mute_audio
				) ;
  ...
  if (member->read_format == AST_FORMAT_SLINEAR)
  if (member->recordfilestream_in)
	{
      ast_writestream(member->recordfilestream_in, f);
	}
  ...
  if (member->read_format == AST_FORMAT_SLINEAR)
  {
   ast_dsp_silence(member->sildet, f, &dspsilence);
   //if (dspsilence)
   //totalsilence = dspsilence;
   //else
   //totalsilence = 0;
   DEBUG("ulex Silence = %d\n\n", dspsilence);
   //ast_log( LOG_WARNING, "ulex Silence = %d\n\n", dspsilence);
  ...
  if ((member->silsup_flag) && (f->frametype == AST_FRAME_VOICE) && (dspsilence > 500))
	{
    ...
    //DEBUG("Process Icomming SLINEAR16 %x \n", AST_FORMAT_SLINEAR16);
		...
		if (
			member->dsp != NULL
#ifndef	AC_USE_G722
      && ((COMPARE_VARFORMAT_IDFORMAT(GET_FRAME_SUBCLASS_FORMAT(f), AST_FORMAT_SLINEAR)))
#else
      && ((COMPARE_VARFORMAT_IDFORMAT(GET_FRAME_SUBCLASS_FORMAT(f), AST_FORMAT_SLINEAR16)))
#endif
	...
	else if (f->frametype == AST_FRAME_VIDEO)
	{
		...
		&& GET_FRAME_CONTROL_TYPE(f) == AST_CONTROL_HANGUP
		)
	{
		// hangup received
		...
		f->frametype == AST_FRAME_CONTROL
		&& GET_FRAME_CONTROL_TYPE(f) == AST_CONTROL_VIDUPDATE
		)
	{
...
#ifdef	VIDEO
#ifdef	TEXT
	else if ( f->frametype == AST_FRAME_TEXT  && (member->does_text || member->digitalsamba))
	{
#if ulex
		if (member->text_relay)
		{
      //ast_log( LOG_WARNING, "Relay  text [%s]\n", CASTDATA2PTR(f->data, char));

			// relay this to the listening channels
			queue_incoming_dtmf_frame( member, f );
		}
		else
		if ( strncmp(CASTDATA2PTR(f->data, char), "changeBroadcast", strlen("changeBroadcast")) == 0 )
		{
      char *ptr = CASTDATA2PTR(f->data, char);
      char *id, *audio, *video;
      int member_id;
      char message[100];
      int res = 0;
      
      //ast_log( LOG_WARNING, "Command  changeBroadcast %s \n", CASTDATA2PTR(f->data, char));
      id = strchr(ptr, ',');
      *id++ = 0;
      audio = strchr(id, ',');
      *audio++ = 0;
      ...
      *video++ = 0;
      ...
      //ast_log( LOG_WARNING, "Command  changeBroadcast %s %s %s %s\n", CASTDATA2PTR(f->data, char), id, audio, video);
      
      member_id = strtol(id, (char **)NULL, 10);
      
      {
        if (audio[0]=='0')
        unmute_member(conf->name, member_id );
        else
        mute_member(conf->name, member_id );
      }
      
      {
        if (video[0]=='0')
        res = video_unmute_member(conf->name, member_id );
        else
        res = video_mute_member(conf->name, member_id );
      }
      
      if (res)
      sprintf(message, "onBroadcastChange,%d,%d,%d", member_id, (audio[0]=='0'), (video[0]=='0'));
      else
      sprintf(message, "onBroadcastChange,%d,%d,%d", member_id, (audio[0]=='0'), 0);
      
      send_text(conf->name, member->id, message);
			//send_text_broadcast(conf->name, message);
    }
    else
		if ( strncmp(CASTDATA2PTR(f->data, char), "changePermission", strlen("changePermission")) == 0 )
		{
      char *ptr = CASTDATA2PTR(f->data, char);
      char *id, *status;
      int member_id;
      ...
      *status++ = 0;
      ...
      if (status[0]=='1')
      change_allow_member(conf->name, member_id );
      else
      change_disallow_member(conf->name, member_id );
    }
    else
		if ( strncmp(CASTDATA2PTR(f->data, char), "kickUser", strlen("kickUser")) == 0 )
		{
      char *ptr = CASTDATA2PTR(f->data, char);
      char *id;
      int member_id;

      //ast_log( LOG_WARNING, "Command  kickUser %s \n", CASTDATA2PTR(f->data, char));
      id = strchr(ptr, ',');
      *id++ = 0;

      //ast_log( LOG_WARNING, "Command  kickUser %s %s\n", CASTDATA2PTR(f->data, char), id);

      member_id = strtol(id, (char **)NULL, 10);

      kick_member(conf->name, member_id );
  	  //member->kick_flag=1;
    }
    else
		if ( strncmp(CASTDATA2PTR(f->data, char), "focusUser", strlen("focusUser")) == 0 )
		{
      char *ptr = CASTDATA2PTR(f->data, char);
      char *id;
      int member_id;
...
static struct ast_frame *get_next_soundframe(struct ast_conf_member *member, struct ast_frame
    *exampleframe) {
    struct ast_frame *f;
	  ...
	  //ast_log( LOG_WARNING, "Ready to go OOO %p, %p\n", member->soundq->vstream, member->soundq->stream);
...
again2:	
    ...
    if (!f)
    if (member->soundq->vstream)
    {
      if (member->soundq->stream)
      member->soundq->stream = NULL;
	...
	if (!toboot->stopped && !toboot->stream && toboot->name[0])
	{
	  //ast_log( LOG_WARNING, "OPEN %s \n", toboot->name);
		toboot->stream = ast_openstream(member->chan, toboot->name, GET_CHAN_LANGUAGE(member->chan));
	  //ast_log( LOG_WARNING, "Ready to go video/audio %p, %p\n", member->soundq->vstream, member->soundq->stream);
#ifdef ulex
		if (toboot->stream)
		{
		  toboot->tv_start = ast_tvnow();
		  toboot->samples = 0;
	    //ast_log( LOG_WARNING, "OPEN VIDEO %s \n", toboot->name);
		  toboot->vstream = ast_openvstream(member->chan, toboot->name, GET_CHAN_LANGUAGE(member->chan));
		}
		else
		toboot->vstream = NULL;
		...
		if (toboot->vstream)
		{
	    //ast_log( LOG_WARNING, "Ready to go video %p, %p\n", member->soundq->vstream, member->soundq->stream);
		  
      //if (member->chan->vstream)
      //ast_closestream(member->chan->vstream);
			SET_CHAN_VSTREAM(member->chan, NULL);
		}
#endif
		...
		{
#ifdef ulex
	    //ast_log( LOG_WARNING, "Ready to go audio %p, %p\n", member->soundq->vstream, member->soundq->stream);

      //if (member->chan->stream)
      //ast_closestream(member->chan->stream);
#endif		
			SET_CHAN_STREAM(member->chan, NULL);
			goto again2;
		}
	...
	if ( ast_set_write_format_by_id( member->chan, member->write_format ) < 0 )
	{
...
// AJO
// get the next frame from the soundq;  must be called with member locked.
static struct ast_frame *get_next_videoframe(struct ast_conf_member *member, struct ast_frame
    *exampleframe) {
   ...
   return NULL;
	 ...
	 //ast_log( LOG_WARNING, "Ready to go %p, %p\n", member->soundq->vstream, member->soundq->stream);
   
   if (member->soundq->vstream == NULL)
   return NULL; 

	 DEBUG("Ready to go video %p, %p\n", member->soundq->vstream, member->soundq->stream);
     
	ast_mutex_unlock(&member->lock);
//again2:
  //f=(member->soundq->vstream && !member->soundq->stopped ? ast_readframe(member->soundq->vstream) : NULL);
  
  f = NULL;
  
  if (member->soundq->vstream && !member->soundq->stopped)
  {
   f=ast_readframe(member->soundq->vstream);   
  }
  ...
  if(!f) { // we're done with this sound; remove it from the queue, and try again
	...
	if (toboot->vstream) {
		ast_closestream(toboot->vstream);
		toboot->vstream = NULL; 
		//ast_log( LOG_WARNING, "finished playing a video: name = %s, stream = %p\n", toboot->name, toboot->stream);
		// notify applications via mgr interface that this sound has been played
		manager_event(
			EVENT_FLAG_CALL,
			"ConferenceVideoComplete",
			"Channel: %s\r\n"
			"Sound: %s\r\n",
			GET_CHAN_NAME(member->chan),
			toboot->name
...
/*
static long usecdiff(struct timeval *starttime, struct timeval *finishtime)
{
  long msec;
...
static int process_outgoing(struct ast_conf_member *member)
{
	...
	if ( member->soundq && member->soundq->vstream)
	{
	  //tv = ast_tvnow();
	  total = ast_tvdiff_ms(ast_tvnow(),member->soundq->tv_start);
		//ast_log( LOG_WARNING, "Total = %d\n", total);
	}
#endif
	...
	for(;;)
	{
		...
		if ( f != realframe )
			...
			if ( ast_check_hangup(member->chan) )
			return 1;
	...
	{	  
		// grab a frame.		
		if ( member->soundq && member->soundq->vstream && total)
		{
			//ast_log( LOG_WARNING, "Frame to Play ?\n");
			
			//ast_log( LOG_WARNING, "TIME : %d , %d \n", member->soundq->samples/90, total);
			
      if ((member->soundq->samples/90) <= total)
			f = get_next_videoframe(member, f);	
			else
			f = NULL;
			
      if (f)
      {
			  //ast_log( LOG_WARNING, "Frame f->samples %d\n",f->samples);
			  member->soundq->samples += f->samples;
			  //ast_log( LOG_WARNING, "Frame soundq->samples %ld : %ld\n",member->soundq->samples, member->soundq->samples* 4/45);
			}
			else
			{
			  // Skip all the buffered frames.
		    while (get_outgoing_video_frame(member));
		    break;
      }			
		}
		else
...
#ifdef ulex
  // Check Wait for moderator
	// unable to find an existing conference, try to create one
	if (!member->ismoderator)
	if (member->waitmoderator)
	if (!check_moderators(conf_name))
	{
    pbx_builtin_setvar_helper(member->chan, "KONFERENCE", "WAITMODERATOR" );
	
		ast_log( LOG_NOTICE, "Moderator not present !\n" ) ;
  ...
  DEBUG("CHANNEL CODECS, WRITEFORMAT => %s \n",ast_getformatname(&member->read_format));
	...
	if ( member->spyee_channel_name != NULL )
	{
		...
		if ( spyee != NULL && spyee->spy_partner == NULL )
		{
			...
			//DEBUG("Start spyer %s, spyee is %s\n", GET_CHAN_NAME(member->chan), member->spyee_channel_name) ;
		} else
		{
			ast_log( LOG_NOTICE, "unable to find spyee member conference %s !\n", member->spyee_channel_name ) ;
	...
	member->bucket = &(channel_table[hash(GET_CHAN_NAME(member->chan)) % CHANNEL_TABLE_SIZE]);

	AST_LIST_LOCK (member->bucket ) ;
	AST_LIST_INSERT_HEAD (member->bucket, member, hash_entry) ;
	AST_LIST_UNLOCK (member->bucket ) ;

	DEBUG("added %s to the channel table, bucket => %d\n", GET_CHAN_NAME(member->chan), member->bucket - channel_table) ;
...
#if AST_FORMAT_SLINEAR16
  member->dsp_threshold = ast_dsp_get_threshold_from_settings(THRESHOLD_SILENCE);
#else
  member->dsp_threshold = 256;
#endif
#ifdef	VIDEO
	member->req_id = -1;
#endif
...
#endif
#ifdef ulex
		static const char arg_id[] = "id";
		static const char arg_req_id[] = "req_id";
		static const char arg_dsp_threshold[] = "dsp_threshold";
#endif
...
#endif
#ifdef ulex
		} else if ( strncasecmp(key, arg_id, sizeof(arg_id) - 1) == 0 )
		{
			member->id = strtol(value, (char **)NULL, 10);
			DEBUG("id = %d\n", member->id) ;
		} else if ( strncasecmp(key, arg_req_id, sizeof(arg_req_id) - 1) == 0 )
		{
			member->req_id = strtol(value, (char **)NULL, 10);
			DEBUG("req_id = %d\n", member->req_id) ;
		} else if ( strncasecmp(key, arg_dsp_threshold, sizeof(arg_dsp_threshold) - 1) == 0 )
		{
			member->dsp_threshold = strtol(value, (char **)NULL, 10);
			DEBUG("dsp_threshold = %d\n", member->dsp_threshold);
#endif
...
#ifdef ulex
	// ulex member flags
	member->waitmoderator = 0;
	
	member->dtmf_exit = 0; // dtmf to exit
	member->dtmf_change = 0; // dtmf to mute/unmute freeze/show
	member->dtmf_unmute = 0; // dtmf to unmute and show

	pbx_builtin_setvar_helper(member->chan, "CONFERENCE_DTMF", "");   

	member->sildet_flag = 0 ;
	member->sildet = NULL;
	
	member->listener_only = 0;
#endif
	...
	for ( i = 0 ; i < strlen( flags ) ; ++i )
	{
...
#endif
		{
			...
			case 'Y':
				...
				break;
			case 'h':
				member->dtmf_change = 1;
				break;
			case 'u':
				member->dtmf_unmute = 1;
				break;
#endif
#ifdef	VIDEO
			case 'X':
			...
			// DSP for VAD
			case 'v':
				member->sildet_flag = 1 ;
				break ;
			case 'n':
				member->silsup_flag = 1 ;
				break ;
#endif
			case 'H':
				member->hold_flag = 1;
...
#ifdef ulex
			case 'b':
				member->listener_only = 1;
				break;
			case 'd':
				member->digitalsamba = 1 ;
				break ;
			case 'p':
				member->promptchange = 1 ;
				break ;
			case 'k':
				member->keep_focus = 1 ;
				break ;
...
#if ( SILDET == 2 )
#ifndef	AC_USE_G722
	member->read_format = ( member->dsp == NULL ) ? GET_CHAN_NATIVEFORMATS_BITFIELD(chan) : AST_FORMAT_SLINEAR ;
#else
	member->read_format = ( member->dsp == NULL ) ? GET_CHAN_NATIVEFORMATS_BITFIELD(chan) : AST_FORMAT_SLINEAR16 ;
#endif
#else
	member->read_format = GET_CHAN_NATIVEFORMATS_BITFIELD(chan) ;
#endif

#ifdef ulex
 if (member->sildet_flag)
	member->read_format = AST_FORMAT_SLINEAR ;
#endif

	member->write_format = GET_CHAN_NATIVEFORMATS_BITFIELD(chan);
...
#ifdef ulex
  if (member->read_format == AST_FORMAT_AMRNB)
	member->read_format = AST_FORMAT_SLINEAR ;
#endif

#ifdef ulex
  if (member->write_format == AST_FORMAT_AMRNB)
	member->write_format = AST_FORMAT_SLINEAR ;
#endif

#ifdef ulex
  if (member->read_format == AST_FORMAT_SPEEX)
	member->read_format = AST_FORMAT_SLINEAR ;
#endif

#ifdef ulex
  if (member->write_format == AST_FORMAT_SPEEX)
	member->write_format = AST_FORMAT_SLINEAR ;
#endif

	// translation paths ( ast_translator_build_path() returns null if formats match )
#if ASTERISK_VERSION_NUM >= AST_11
	struct ast_format formatread;
	struct ast_format formatwrite;
	struct ast_format formatlinear;
#ifndef	AC_USE_G722
	ast_format_set(&formatlinear, AST_FORMAT_SLINEAR, 0);
#else
	ast_format_set(&formatlinear, AST_FORMAT_SLINEAR16, 0);
#endif
	ast_format_from_old_bitfield(&formatread, member->read_format);
	ast_format_from_old_bitfield(&formatwrite, member->write_format);
	member->to_slinear = ast_translator_build_path( &formatlinear, &formatread ) ;
	member->from_slinear = ast_translator_build_path( &formatwrite, &formatlinear ) ;
#else
#ifndef	AC_USE_G722
  ...
  if (member->sildet_flag) {
   ...
   //return -1;
   //}
   member->sildet = ast_dsp_new();
   if (!member->sildet) {
    ast_log(LOG_WARNING, "Unable to create silence detector :(\n");
    //return -1;
   }
   ast_dsp_set_threshold(member->sildet, member->dsp_threshold);
  }  
    ...
    channel_name = ast_strdupa(GET_CHAN_NAME(chan));
    while ((p = strchr(channel_name, '/')))
      ...
      *p = '-';
		...
		if (member->id != -1)
    sprintf(filename, "/tmp/konference_%s_%d_in", conf_name, member->id);
		else
    sprintf(filename, "/tmp/konference_%s_%s_in", conf_name, channel_name);
    member->recordfilestream_in = ast_writefile(filename, "wav", NULL, O_CREAT|O_WRONLY|O_TRUNC, 0, 0666 );

		if (member->id != -1)
		sprintf(filename, "/tmp/konference_%s_%d_out", conf_name, member->id);
		else
    sprintf(filename, "/tmp/konference_%s_%s_out", conf_name, channel_name);
    member->recordfilestream_out = ast_writefile(filename, "wav", NULL, O_CREAT|O_WRONLY|O_TRUNC, 0, 0666);
	}
	...
	DEBUG("created member, type => %s, priority => %d, readformat => %llx\n", member->type, member->priority, FROM_READFORMAT_TO_OLDREADFORMAT(chan)) ;
...
#ifdef ulex
	if (member->sildet)
  ast_dsp_free(member->sildet);

	if (member->recordfilestream_in)
  ast_closestream(member->recordfilestream_in);

	if (member->recordfilestream_out)
  ast_closestream(member->recordfilestream_out);
#endif
...
conf_frame* get_incoming_frame( struct ast_conf_member *member )
{
	...
	{
		DEBUG("enabling cached frame, channel => %s, incoming => %d, outgoing => %d\n", GET_CHAN_NAME(member->chan), member->inFramesCount, member->outFramesCount) ;
...
int queue_incoming_video_frame( struct ast_conf_member* member, const struct ast_frame* fr )
{
	...
	{
		DEBUG("unable to queue incoming VIDEO frame, channel => %s, incoming => %d, outgoing => %d\n", GET_CHAN_NAME(member->chan), member->inVideoFramesCount, member->outVideoFramesCount) ;
		ast_mutex_unlock(&member->lock);
		return -1 ;
	}
...
int queue_incoming_dtmf_frame( struct ast_conf_member* member, const struct ast_frame* fr )
{
	...
	{
		DEBUG("unable to queue incoming DTMF frame, channel => %s, incoming => %d, outgoing => %d\n", GET_CHAN_NAME(member->chan), member->inDTMFFramesCount, member->outDTMFFramesCount) ;
		ast_mutex_unlock(&member->lock);
		return -1 ;
	}
...
int __queue_outgoing_frame( struct ast_conf_member* member, const struct ast_frame* fr, struct timeval delivery )
{
	...
	if (member->recordfilestream_out)
	{
    ast_writestream(member->recordfilestream_out, (struct ast_frame *)fr);
	}
...
int queue_outgoing_text_frame( struct ast_conf_member* member, const struct ast_frame* fr)
{
	...
	{
		DEBUG("unable to queue outgoing text frame, channel => %s, incoming => %d, outgoing => %d\n", GET_CHAN_NAME(member->chan), member->inTextFramesCount, member->outTextFramesCount) ;
...
)
{
  int c;
	...
	if ( qf == NULL )
	{
		...
		ast_format_from_old_bitfield(&dst, member->write_format);
		struct ast_trans_pvt* trans = ast_translator_build_path(&dst, &src);
#else
		...
		struct ast_format src;
		struct ast_format dst;
		ast_format_set(&src, AST_FORMAT_SLINEAR16, 0);
		ast_format_from_old_bitfield(&dst, member->write_format);
		struct ast_trans_pvt* trans = ast_translator_build_path(&dst, &src);
#else
	...
	)
{
	...
	{
		DEBUG("channel => %s, inFramesNeeded => %d, inFramesCount => %d\n", GET_CHAN_NAME(member->chan), member->inFramesNeeded, member->inFramesCount) ;
	}
#endif
