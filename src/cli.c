...
static char conference_version_usage[] =
	...
	"       Display vonference version\n"
;

#define CONFERENCE_VERSION_CHOICES { "vonference", "version", NULL }
static char conference_version_summary[] = "Display vonference version";
...
#else
static char conference_version_command[] = "vonference version";
char *conference_version(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VERSION_CHOICES;
	NEWCLI_SWITCH(conference_version_command,conference_version_usage)
#endif
	if ( argc < 2 )
		return SHOWUSAGE ;

	ast_cli( fd, "app_vonference revision %s\n", REVISION) ;
...
#define CONFERENCE_RESTART_CHOICES { "vonference", "restart", NULL }
...
#else
static char conference_restart_command[] = "vonference restart";
char *conference_restart(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_RESTART_CHOICES;
...
#define CONFERENCE_DEBUG_CHOICES { "vonference", "debug", NULL }
...
#else
static char conference_debug_command[] = "vonference debug";
char *conference_debug(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_DEBUG_CHOICES;
...
#define CONFERENCE_SHOW_STATS_CHOICES { "vonference", "show", "stats", NULL }
...
#else
static char conference_show_stats_command[] = "vonference show stats";
char *conference_show_stats(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_SHOW_STATS_CHOICES;
	...
	for ( i = 0 ; i < count ; ++i )
	{
		...
		ast_cli( fd, "%-40.40s\n", (char*)( &(s->name) )) ;
		ast_cli( fd, " Moderators      : %d\n", s->moderators) ;
		ast_cli( fd, " Conferees       : %d\n", s->conferees) ;
		ast_cli( fd, " Frames in       : %ld\n", s->frames_in) ;
		ast_cli( fd, " Frames out      : %ld\n", s->frames_out) ;
		ast_cli( fd, " Frames mixed    : %ld\n", s->frames_mixed) ;
		ast_cli( fd, " Time            : %lld secondes\n", ast_tvdiff_ms(ast_tvnow(), s->time_entered) / 1000) ;
	}
...
#define CONFERENCE_LIST_CHOICES { "vonference", "list", NULL }
...
#else
static char conference_list_command[] = "vonference list";
char *conference_list(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_LIST_CHOICES;
...
static char conference_kick_usage[] =
	"Usage: vonference kick <conference> <member id>\n"
	"       Kick member <member id> from conference <conference>\n"
;

#define CONFERENCE_KICK_CHOICES { "vonference", "kick", NULL }
...
#else
static char conference_kick_command[] = "vonference kick";
char *conference_kick(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_KICK_CHOICES;
...
#define CONFERENCE_KICKCHANNEL_CHOICES { "vonference", "kickchannel", NULL }
...
#else
static char conference_kickchannel_command[] = "vonference kickchannel";
char *conference_kickchannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_KICKCHANNEL_CHOICES;
...
#define CONFERENCE_MUTE_CHOICES { "vonference", "mute", NULL }
...
#else
static char conference_mute_command[] = "vonference mute";
char *conference_mute(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_MUTE_CHOICES;
...
#define CONFERENCE_MUTECONFERENCE_CHOICES { "vonference", "muteconference", NULL }
...
#else
static char conference_muteconference_command[] = "vonference muteconference";
char *conference_muteconference(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_MUTECONFERENCE_CHOICES;
...
#define CONFERENCE_MUTECHANNEL_CHOICES { "vonference", "mutechannel", NULL }
...
#else
static char conference_mutechannel_command[] = "vonference mutechannel";
char *conference_mutechannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_MUTECHANNEL_CHOICES;
...
#define CONFERENCE_VIEWSTREAM_CHOICES { "vonference", "viewstream", NULL }
...
#else
static char conference_viewstream_command[] = "vonference viewstream";
char *conference_viewstream(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIEWSTREAM_CHOICES;
  ...
  ast_cli( fd, "User #: %d viewing %d\n", member_id, viewstream_id) ;
	...
	ast_cli( fd, "Fail swithcing user #: %d viewing %d\n", member_id, viewstream_id) ;
...
#define CONFERENCE_VIEWCHANNEL_CHOICES { "vonference", "viewchannel", NULL }
...
#else
static char conference_viewchannel_command[] = "vonference viewchannel";
char *conference_viewchannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIEWCHANNEL_CHOICES;
...
#define CONFERENCE_UNMUTE_CHOICES { "vonference", "unmute", NULL }
...
#else
static char conference_unmute_command[] = "vonference unmute";
char *conference_unmute(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_UNMUTE_CHOICES;
...
#define CONFERENCE_UNMUTECONFERENCE_CHOICES { "vonference", "unmuteconference", NULL }
...
#else
static char conference_unmuteconference_command[] = "vonference unmuteconference";
char *conference_unmuteconference(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_UNMUTECONFERENCE_CHOICES;
...
#define CONFERENCE_UNMUTECHANNEL_CHOICES { "vonference", "unmutechannel", NULL }
...
#else
static char conference_unmutechannel_command[] = "vonference unmutechannel";
char *conference_unmutechannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_UNMUTECHANNEL_CHOICES;
...
static char conference_play_sound_usage[] =
	"Usage: vonference play sound <channel> (<sound-file>)+ [mute|tone]\n"
...
#define CONFERENCE_PLAY_SOUND_CHOICES { "vonference", "play", "sound", NULL }
...
#else
static char conference_play_sound_command[] = "vonference play sound";
char *conference_play_sound(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_PLAY_SOUND_CHOICES;
...
// playall sound
//
static char conference_playall_sound_usage[] =
	"usage: vonference playall sound <conference> <sound-file> [mute]\n"
	"       play sound <sound-file> to the members of the conference <conference>.\n"
	"       If mute is specified, all other audio is muted while the sound is played back.\n"
;

#define CONFERENCE_PLAYALL_SOUND_CHOICES { "vonference", "playall", "sound", NULL }
static char conference_playall_sound_summary[] = "Play a sound to all conference members";

#ifndef AST_CLI_DEFINE
static struct ast_cli_entry cli_playall_sound = {
	CONFERENCE_PLAYALL_SOUND_CHOICES,
	conference_playall_sound,
	conference_playall_sound_summary,
	conference_playall_sound_usage
} ;
int conference_playall_sound( int fd, int argc, char *argv[] ) {
#else
static char conference_playall_sound_command[] = "vonference playall sound";
char *conference_playall_sound(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_PLAYALL_SOUND_CHOICES;
	NEWCLI_SWITCH(conference_playall_sound_command,conference_playall_sound_usage)
#endif
	...
	int res = play_sound_conference(fd, name, file, mute);

	if ( !res )
	{
		ast_cli(fd, "Sound playall failed failed\n");
		return FAILURE;
	}
	return SUCCESS ;
}
...
static char conference_stop_sounds_usage[] =
	"Usage: vonference stop sounds <channel>\n"
	"       Stop sounds for conference member <channel>\n"
;

#define CONFERENCE_STOP_SOUNDS_CHOICES { "vonference", "stopall", "sounds", NULL }
...
#else
static char conference_stop_sounds_command[] = "vonference stop sounds";
char *conference_stop_sounds(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_STOP_SOUNDS_CHOICES;
...
// stopall sound
//
static char conference_stopall_sounds_usage[] =
	"usage: vonference stopall sounds <conference>\n"
	"       stopall sounds to the members of the conference <conference>.\n"
	"       If mute is specified, all other audio is muted while the sound is played back.\n"
;

#define CONFERENCE_STOPALL_SOUNDS_CHOICES { "vonference", "playall", "sounds", NULL }
static char conference_stopall_sounds_summary[] = "Stop sounds for all conference members";

#ifndef AST_CLI_DEFINE
static struct ast_cli_entry cli_stopall_sounds = {
	CONFERENCE_STOPALL_SOUNDS_CHOICES,
	conference_stopall_sounds,
	conference_stopall_sounds_summary,
	conference_stopall_sounds_usage
} ;
int conference_stopall_sounds( int fd, int argc, char *argv[] ) {
#else
static char conference_stopall_sounds_command[] = "vonference stopall sounds";
char *conference_stopall_sounds(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_STOPALL_SOUNDS_CHOICES;
	NEWCLI_SWITCH(conference_stopall_sounds_command,conference_stopall_sounds_usage)
#endif
	const char *name;
	...
	int res = stop_sound_conference(fd, name);

	if ( !res )
	{
		ast_cli(fd, "Sounds stopall failed failed\n");
		return FAILURE;
	}
	return SUCCESS ;
}
...
#define CONFERENCE_START_MOH_CHOICES { "vonference", "start", "moh", NULL }
...
#else
static char conference_start_moh_command[] = "vonference start moh";
char *conference_start_moh(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_START_MOH_CHOICES;
...
static char conference_stop_moh_usage[] =
	"Usage: vonference stop moh <channel>\n"
	"       Stop moh for conference member <channel>\n"
;

#define CONFERENCE_STOP_MOH_CHOICES { "vonference", "stop", "moh", NULL }
...
#else
static char conference_stop_moh_command[] = "vonference stop moh";
char *conference_stop_moh(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_STOP_MOH_CHOICES;
...
#define CONFERENCE_TALKVOLUME_CHOICES { "vonference", "talkvolume", NULL }
...
#else
static char conference_talkvolume_command[] = "vonference talkvolume";
char *conference_talkvolume(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_TALKVOLUME_CHOICES;
...
static char conference_listenvolume_usage[] =
	"Usage: vonference listenvolume <channel> ( up | down )\n"
	"       Adjust listen volume for conference member <channel>\n"
;

#define CONFERENCE_LISTENVOLUME_CHOICES { "vonference", "listenvolume", NULL }
...
#else
static char conference_listenvolume_command[] = "vonference listenvolume";
char *conference_listenvolume(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_LISTENVOLUME_CHOICES;
...
#define CONFERENCE_VOLUME_CHOICES { "vonference", "volume", NULL }
...
#else
static char conference_volume_command[] = "vonference volume";
char *conference_volume(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VOLUME_CHOICES;
...
static char conference_end_usage[] =
	"Usage: vonference end <conference name>\n"
	"       Ends a conference\n"
;

#define CONFERENCE_END_CHOICES { "vonference", "end", NULL }
...
#else
static char conference_end_command[] = "vonference end";
char *conference_end(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_END_CHOICES;
...
#define CONFERENCE_LOCK_CHOICES { "vonference", "lock", NULL }
...
#else
static char conference_lock_command[] = "vonference lock";
char *conference_lock(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_LOCK_CHOICES;
...
#define CONFERENCE_LOCKCHANNEL_CHOICES { "vonference", "lockchannel", NULL }
...
#else
static char conference_lockchannel_command[] = "vonference lockchannel";
char *conference_lockchannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_LOCKCHANNEL_CHOICES;
...
#define CONFERENCE_UNLOCK_CHOICES { "vonference", "unlock", NULL }
...
#else
static char conference_unlock_command[] = "vonference unlock";
char *conference_unlock(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_UNLOCK_CHOICES;
...
static char conference_set_default_usage[] =
	"Usage: vonference set default <conference name> <member id>\n"
...
#define CONFERENCE_SET_DEFAULT_CHOICES { "vonference", "set", "default", NULL }
...
#else
static char conference_set_default_command[] = "vonference set default";
char *conference_set_default(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_SET_DEFAULT_CHOICES;
...
#define CONFERENCE_SET_DEFAULTCHANNEL_CHOICES { "vonference", "set", "defaultchannel", NULL }
...
#else
static char conference_set_defaultchannel_command[] = "vonference set defaultchannel";
char *conference_set_defaultchannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_SET_DEFAULTCHANNEL_CHOICES;
...
static char conference_video_mute_usage[] =
	"Usage: vonference video mute <conference name> <member id>\n"
	"       Mutes video from member <member id> in conference <conference name>\n"
;

#define CONFERENCE_VIDEO_MUTE_CHOICES { "vonference", "video", "mute", NULL }
...
#else
static char conference_video_mute_command[] = "vonference video mute";
char *conference_video_mute(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIDEO_MUTE_CHOICES;
...
static char conference_video_unmute_usage[] =
	"Usage: vonference video unmute <conference name> <member id>\n"
	"       Unmutes video from member <member id> in conference <conference name>\n"
;

#define CONFERENCE_VIDEO_UNMUTE_CHOICES { "vonference", "video", "unmute", NULL }
...
#else
static char conference_video_unmute_command[] = "vonference video unmute";
char *conference_video_unmute(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIDEO_UNMUTE_CHOICES;
...
#define CONFERENCE_VIDEO_MUTECHANNEL_CHOICES { "vonference", "video", "mutechannel", NULL }
...
#else
static char conference_video_mutechannel_command[] = "vonference video mutechannel";
char *conference_video_mutechannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIDEO_MUTECHANNEL_CHOICES;
...
#define CONFERENCE_VIDEO_UNMUTECHANNEL_CHOICES { "vonference", "video", "unmutechannel", NULL }
...
#else
static char conference_video_unmutechannel_command[] = "vonference video unmutechannel";
char *conference_video_unmutechannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_VIDEO_UNMUTECHANNEL_CHOICES;
...
static char conference_text_usage[] =
	"Usage: vonference text <conference name> <member id> <text>\n"
	"        Sends text message <text> to member <member id> in conference <conference name>\n"
;

#define CONFERENCE_TEXT_CHOICES { "vonference", "text", NULL }
...
#else
static char conference_text_command[] = "vonference text";
char *conference_text(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_TEXT_CHOICES;
...
#define CONFERENCE_TEXTCHANNEL_CHOICES { "vonference", "textchannel", NULL }
...
#else
static char conference_textchannel_command[] = "vonference textchannel";
char *conference_textchannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_TEXTCHANNEL_CHOICES;
...
#define CONFERENCE_TEXTBROADCAST_CHOICES { "vonference", "textbroadcast", NULL }
...
#else
static char conference_textbroadcast_command[] = "vonference textbroadcast";
char *conference_textbroadcast(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_TEXTBROADCAST_CHOICES;
...
#define CONFERENCE_DRIVE_CHOICES { "vonference", "drive", NULL }
...
#else
static char conference_drive_command[] = "vonference drive";
char *conference_drive(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_DRIVE_CHOICES;
...
#define CONFERENCE_DRIVECHANNEL_CHOICES { "vonference", "drivechannel", NULL }
...
#else
static char conference_drivechannel_command[] = "vonference drivechannel";
char *conference_drivechannel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
	static const char *choices[] = CONFERENCE_DRIVECHANNEL_CHOICES;
...
#ifdef AST_CLI_DEFINE
static struct ast_cli_entry app_vonference_commands[] = {
...
#ifdef ulex
	AST_CLI_DEFINE(conference_playall_sound, conference_playall_sound_summary),
	AST_CLI_DEFINE(conference_stopall_sounds, conference_stopall_sounds_summary),
#endif	
...
#ifdef ulex
	ast_cli_register( &cli_playall_sound ) ;
#endif	
...
{
#ifdef AST_CLI_DEFINE
	ast_cli_unregister_multiple(app_vonference_commands, sizeof(app_vonference_commands)/sizeof(struct ast_cli_entry));
#else
...
#ifdef ulex
	ast_cli_unregister( &cli_playall_sound ) ;
#endif		
...
#endif
#ifdef	MANAGER_COMMANDS
	ast_manager_unregister( "VonferenceList" );
	ast_manager_unregister( "VonferenceEnd" );
#endif
}
