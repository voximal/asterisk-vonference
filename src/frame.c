...
// $Id: frame.c,v 1.1 2016/05/26 08:45:43 borja.sixto Exp $
...
#ifdef ulex			
			if (
			  ( cf_spoken->member != NULL)
        && (cf_send->member != NULL)
        && (cf_send->member->listener_only && cf_spoken->member->norecv_audio))
			{
				// don't mix this frame
				// (Only here the members how receive audio)
      }
      else
#endif
			//
...
#else
	SET_FRAME_SUBCLASS_FORMAT(f, AST_FORMAT_SLINEAR16);
#endif
